...
If you have an existing Java Gradle project, add the snippet shown below to the `cicsBundle` and `dependencies` sections of your `build.gradle` and edit the variables as needed. If you're deploying the bundle into a single region environment (SMSS), remove the `cicsplex` and `region` fields. Your Gradle project should now resemble the sample.  
```gradle
cicsBundle {
    build {
